...
```{r}
flows_filt <- flows %>% 
  filter(site_no != "06192980", site_no != "06187915") %>% 
...
```{r}
flows_filt %>% 
  ggplot(aes(x = date, y = flow_cfs, color = site_no)) +
  geom_point() +
  facet_wrap(facets = "site_no")
```
...
```{r}
site_drain <- site_info %>% 
  select(site_no, station_nm, drain_area_va)

flow_mm <- left_join(flows_filt, site_drain, by = "site_no") 

flow_mm <- flow_mm %>% 
  rename(drain_area_sq_mi = drain_area_va) %>% 
  mutate(drain_area_sq_me = drain_area_sq_mi*2.59e+6) %>% 
  mutate(q_mm_d = ((0.0283 * flow_cfs * 86400)/drain_area_sq_me)*1000)
```
...
```{r}
flow_mm %>% 
  ggplot(aes(x = date, y = q_mm_d, color = station_nm)) +
  geom_line() +
  facet_wrap(facets = "site_no")
```
...
flow_mm_an <- flow_mm %>% 
  ...
  reframe(tot_an_mm = sum(q_mm_d, na.rm = TRUE)) 
...
```{r}
flow_mm_an %>% 
  ggplot(aes(x = year, y = tot_an_mm, color = station_nm)) +
  geom_line(linetype = "dashed") +
  geom_point() +
  labs(x = "", y = "Total annual flow (mm/y)", color = "Gauge") +
  theme_linedraw()
```

Make a map of the gauges included in this plot of total annual flow to aid in interpretation. 
```{r}
gauges_filt <- stream_gauges %>% 
  filter(site_no != "06192980", site_no != "06187915") 

leaflet() %>% addProviderTiles("Stamen.Terrain") %>% 
  addCircleMarkers(data = gauges_filt, lat = ~dec_lat_va, lng = ~dec_long_va, label = ~station_nm, 
                    popup = ~paste("Site", site_no)) 
